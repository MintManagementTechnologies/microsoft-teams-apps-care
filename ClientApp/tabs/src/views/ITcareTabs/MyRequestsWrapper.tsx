...
import { ITicketModel, ITicketViewModel } from '../../features/tickets/types';
import { useAppDispatch, useTypedSelector, RootState } from '../../store';
import BrowseTickets from './BrowseTickets';

const MyRequestsWrapper = (): JSX.Element => {
   ...
   useEffect(() => {
      ...
      if (currentUserIsTech === true) {
         processItems = processItems.filter(x => x.assignedToUPN.toLowerCase() === currentCtx.upn.toLowerCase());
      } else {
         processItems = processItems.filter(x => x.requesterUPN.toLowerCase() === currentCtx.upn.toLowerCase());
      }  
